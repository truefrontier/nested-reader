...
The store keeps `working[key]` per stream (`lookup:<id>`, `page:<path>`, `refine:<path>`) from those events, clearing it on the next text delta and at the end. Each answer card shows the line in place of its empty answer, a page being written shows it under its skeleton (`.tool-line`), and each refine status card adds it under the instruction.
...
`ui.refining`, `ui.refineText` and `ui.refineError` are keyed by path, so each page being refined drives a status card of its own in the place of the refine box (in the page for a selection, at the foot of the panes for page and corpus scope): two refinements at once read their own instruction and their own failure, and a page whose refinement failed keeps the text for Try again while its neighbour reviews. A corpus refine is one thing the reader asked for, so it keeps a single card on the page it was asked from until every page has come back. `refineRuns` counts what each page has running, so a refinement queued behind another holds the card up rather than letting the first one take it down. The cards and the pane-level boxes share one column (`.pane-stack`), so a second refinement can be typed while the first still runs. The ask and refine boxes share the typed draft, held by the page, across the ⌘R switch.
