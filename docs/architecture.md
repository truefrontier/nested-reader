...
| `.reader/session.json` | Current page, read/unread state, trail, split, pending reviews, a display name if the session was renamed, `ids` (Unix `dev:ino` per page path) so a Finder rename can be remapped after quit, and, once a nested file or subfolder of an added root has been taken out of the tree with Remove from session, `excluded` (the keys it dropped, in the same shape `roots` pages are keyed). |
...
A single page or a whole subfolder nested inside an added root can also be dropped on its own, leaving the rest of that root in place: the sidebar ··· menu on a nested file or folder row offers Remove from session too (`excludeFromSession` in the store), which appends the row's key to `session.excluded` and reloads; `rootPages` then leaves out any page whose key equals an entry or sits under one (`key === e || key.startsWith(e + "/")`). The file or folder itself is never touched — only the session's view of it changes. `excluded` is pruned alongside `roots` on removal and on reload (an entry whose root is gone or was itself removed has nothing left to hide), so it never outlives the root it belongs to; adding the same folder back as a root later brings the previously-excluded pages with it, since nothing on disk remembers the exclusion.

With tools on, `AiRequest.roots` carries the added folders: `tools.rs` lists their pages by full path and reads and searches them alongside the session folder, and the Claude CLI gets one `--add-dir` per folder. `AiRequest.excluded` carries `session.excluded` the same way, and `tools.rs` filters every excluded key (and anything nested under it) out of `list_pages`, `read_page` and `search_pages`, so a page removed from the session is as unreachable to the model's own tools as a dropped root's pages are. The Claude CLI's `--add-dir` and Codex's read-only sandbox grant the whole folder, though, with no way to carve a path back out of that grant; for them the excluded keys are named in the system-prompt note (`folder_note` in `cli.rs`) as pages the model must leave alone, which is an instruction rather than a hard block.
...
Theme, text size, reading font, page width and page position are settings. `applyTheme` in the store writes them to the root element as `data-theme`, `--text-size`, `data-font`, `--reading-width` and `data-align`, at launch and again whenever the settings window saves. The reading column (`.article`) takes `--reading-width` as its `max-width`. The unit is `em`, emitted as `calc(n * var(--text-size))` rather than a bare `em` length, so the width still follows the text size but resolves to the same number of pixels wherever the token is read, including on `.top`, whose own font is 12px; or `%` of the pane. The setting keeps a value per unit (`readingWidth: { unit, em, percent }`), so switching units brings back the last choice made in each. The default is 33em, the old 560px at 17px text, so an upgrade moves nothing. Values outside 20–60em or 30–100% are clamped when applied.

Page position is `pageAlign`, `"left"` or `"center"`, and the page's text stays left-justified either way; only its box moves. Left is what shipped: the box sits against the pane's left edge, its 64px of gutter split 48px before the text and 16px after. `:root[data-align="center"]` turns `--page-margin` to `auto`, so the box's own margins split the pane's slack, and halves that gutter to 32px either side, so the column keeps its width and sits centred in its box rather than pushed to the left of it. `.top` carries the same `max-width` and margin as the article, so the review strip, the version pill and the find bar keep sitting over the page's left edge in both. The split pane follows through the inherited `--page-margin` and keeps its own 28px inset, which is right in either position; at its usual 26% the width never binds, so the alignment only shows when ⌘⇧F gives it the whole window.
